...
class Block:
        ...
        if len(self.children) == 0:
            ...
            for i in range(4):
                if len(self.children[i].children) == 0:
                    self.children[i].position = children_positions[i]
                else:
    ...
    def smash(self) -> bool:
        ...
        if not self.smashable():
            # this means that we can't smash this block
            return False
        ...
        self.colour = None
        for i in range(4):
            different_color = random.choice(COLOUR_LIST)
            b = Block(positions[i], size, different_color, level, depth)
    ...
    def swap(self, direction: int) -> bool:
        ...
        if direction == 0:
            ...
            # now updating children positions
    ...
    def paint(self, colour: Tuple[int, int, int]) -> bool:
        ...
        """
        if not self.level == self.max_depth and len(self.children) != 0:
            # this is not a leaf or it's not in max depth
            return False
        if self.colour == colour:
            # leaf has the same color; no need to paint it
            return False
        self.colour = colour
        return True
    ...
    def combine(self) -> bool:
        ...
        if self.level != self.max_depth - 1 or len(self.children) == 0:
            # children are not at max level or block has no children
            return False

        lst_of_colors = {}

        for child in self.children:
            if child.colour in lst_of_colors:
                lst_of_colors[child.colour] += 1
            else:
                lst_of_colors[child.colour] = 1

        count = 0
        value = lst_of_colors.values()
        ...
        if count == 1:
            self.children = []
            target = max(lst_of_colors, key=lst_of_colors.get)
            self.colour = target
            return True
    ...
    def create_copy(self) -> Block:
        ...
        new_block = Block(position, size, color, level, max_depth)
        if len(self.children) == 0:
            return new_block
        else:
            new_children = []
